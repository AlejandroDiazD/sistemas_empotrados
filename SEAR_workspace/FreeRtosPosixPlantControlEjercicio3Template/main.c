...
//El monitor debe poder identificar que el dato es del teclado
	struct monitorMsg msg;

	msg.signal = SKeybChar;
	msg.data = Key;

	BaseType_t xQueueSend ( QueueHandle_t MonitorQueue,
					const void * msg,
...
void MonitorTask( void *pvParmeters )
{
   ...
   struct monitorMsg_t newMsg;
    ...
    while(1)
    {
    	...
    	BaseType_t xQueueReceive (QueueHandle_t MonitorQueue,
    					const void * pvBuffer,
...
int main(void) {
  ...
  //TODO Crear la cola MonitorQueue
	queueHandle = xQueueCreate(
					(UBaseType_t) 10,
					(UBaseType_t) sideof(struct monitorMsg_t));
  ...
  //TODO Declara handler para la tarea Monitor.
	...
	xTaskCreate(MonitorTask, (const char *) "MTASK", 128, NULL, 0, &queueHandle);
	vTaskStartScheduler();
